...
mongodb_dbname = os.getenv('MONGODB_DBNAME', 'exam_system')
...
def parse_iso_datetime_utc(iso_string):
    if not iso_string:
        return None
...
def create_test():
    ...
    if request.method == 'POST':
        ...
        exam_date_utc = request.form.get('exam_date_utc')
        if exam_date_utc:
            exam_date = parse_iso_datetime_utc(exam_date_utc)
        else:
...
def leaderboard():
    ...
    sorted_students = sorted(
        ...
        key=lambda x: (
            -x['total_score'],
            x['fastest_time'] if x['fastest_time'] != float('inf') else float('inf')
        )
    )
    ...
    for test_id in completed_test_ids:
        try:
            ...
            if test:
                test_submission_count = len([
                    s for s in completed_submissions
